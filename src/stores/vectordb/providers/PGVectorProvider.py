...
from models.db_shcemes import RetrievedDocument
...
class PGVectorProvider(VectorDBInterface):
    ...
    async def is_collection_exists(self, collection_name) -> bool:
        ...
        async with self.db_client() as session:
            async with session.begin():
                table_info_sql = sql_text('''
                    SELECT schemename, tablename, tableowner, tablespace, hasindexes
                    FROM pgtables
    ...
    async def insert_many(self, collection_name, texts, vectors, metadata = None, record_ids = None, batch_size = 50):
        ...
        if not len(metadata) or len(metadata)== 0:
